...
m = df[~df['message'].isna()]
m['myTag'] = m['message'].str.contains('@Niva228', na = False, case = False, regex = True)
stat = m[m['myTag']].groupby('dialog_id', as_index = False).count()[['dialog_id', 'myTag']].sort_values(by='myTag', ascending=False).iloc[:10]
stat['dialog_id'] = stat['dialog_id'].astype('str')
sns.barplot(x=stat['dialog_id'], y=stat['myTag'],)
...
tab = sticker.groupby('dialog_id', as_index = False)['type']
tab = tab.count().sort_values(by='type', ascending=False).iloc[:20]
...
m = df[df['type']=='text']
stat = m.groupby('dialog_id', as_index = False)['type']
stat = stat.count().sort_values(by='type', ascending=False).iloc[:20]
plt.figure(figsize = (8,5))
stat['dialog_id'] = stat['dialog_id'].astype('str')
sns.barplot(x=stat["dialog_id"], y=stat['type'],)
plt.ylabel('messages')
plt.xticks(rotation=90)
plt.show()
...
tab = m[m['cs']].groupby('dialog_id', as_index = False).count()[['dialog_id', 'cs']].sort_values(by='cs', ascending=False).iloc[:5]
tab['dialog_id'] = tab['dialog_id'].astype('str')
plt.figure(figsize = (8,5))
sns.barplot(x=tab["dialog_id"], y=tab['cs'],)
plt.ylabel('К С')
plt.xticks(rotation=90)
plt.show()
